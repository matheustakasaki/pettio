    ...
    private void Notificação(){
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O) {
            NotificationChannel channel =
                    new NotificationChannel("LembreteAgua" ,"lembreteAgua" ,
                            NotificationManager.IMPORTANCE_DEFAULT);
            NotificationManager nm = (NotificationManager)getSystemService(Context.NOTIFICATION_SERVICE);
            nm.createNotificationChannel(channel);}

        NotificationCompat.Builder builder = new NotificationCompat.Builder(this,"Lembrete");
        builder.setSmallIcon(R.drawable.agua);
        builder.setContentTitle("Não vá deixa-lo com sede em !");
        builder.setContentText("Não esqueça de dar água ao seu pet :)");
